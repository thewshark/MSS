	...
	OPEN curCabBo
		...
		WHILE @@FETCH_STATUS = 0 BEGIN
			BEGIN TRY
				...
				select @contImp = count(1) from ts inner join ts2 on ts.tsstamp=ts2stamp where ts2.docsimport=1 and ndos=@DocSerie
				...
				BEGIN
					...
					SET @DOC = @DCCEXR + ' ' + @DCCTPD + ' ' + @DCCSER + '/' + CAST(@DCCNDC-1 as VARCHAR(10)) + ' BOSTAMP = ' + @bostamp
